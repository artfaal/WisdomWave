...
async def ask_openai(message: types.Message, text: str, chat_type: str, group_title=None):
    ...
    # Определение пороговых значений в процентах и количестве удаляемых сообщений
    SOFT_LIMIT = 0.9 * MAX_TOKENS
    HARD_LIMIT = 0.95 * MAX_TOKENS
    ...
    # Если требуется удаление сообщений, отправляем уведомление и удаляем сообщения
    if messages_to_delete > 0:
        warning_text = f"""
ℹ️ Внимание!
Вы приближаетесь к максимальному количеству токенов.
Я освобожу место для новых сообщений, удалив {messages_to_delete} первых сообщений из истории.
Рекомендую отправить команду \"забудь\", что бы обнулить счетчик, если контекст диалога не важен.
"""
        await bot.send_message(
            chat_id=message.chat.id,
            text=warning_text,
